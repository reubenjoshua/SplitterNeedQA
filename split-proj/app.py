...
def generate_report():
    try:
        ...
        app.logger.info(f"Received data - original_filename: {original_filename}")
        ...
        try:
                            ...
                            if payment_mode == 'SM' and original_line:
                                ...
                                if payment_mode == 'BDO' and len(raw_row) > 2:
                                    ...
                                    if date_str:
                                        ...
                                        # Use the date as is since it's already formatted in the txt file
                                        dates.add(date_str)
            ...
            for atm_ref, transactions in processed_data.items():
                ...
                if payment_mode == 'ROB' and transactions and isinstance(transactions[0], dict):
                    ...
                    report_path = os.path.join(temp_dir, f'ATM_{actual_atm_ref}_{payment_mode}.txt')
                else:
                    report_path = os.path.join(temp_dir, f'ATM_{atm_ref}_{payment_mode}.txt')
            ...
            response.headers['Content-Disposition'] = f'attachment; filename="{original_filename}.zip"'

            # Log the final filename for debugging
            app.logger.info(f"Generated zip file: {original_filename}.zip")
    ...
    try:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                lines = content.split('\n')[:5]  # Get first 5 lines for validation

                # Validate file format based on payment mode
                is_valid = False
                error_message = f"File does not match {payment_mode} format"

                if payment_mode == 'BDO':
                    # BDO files should have pipe-separated values with specific fields
                    for line in lines:
                        if '|' in line and len(line.split('|')) >= 10:
                            is_valid = True
                            break
                    if not is_valid:
                        error_message = "BDO files should contain pipe-separated values with at least 10 fields"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                    ...
                    # UNIONBANK files should have specific format with UB reference
                    for line in lines:
                        if 'UB' in line and re.search(r'\d{14}', line):
                            is_valid = True
                            break
                    if not is_valid:
                        error_message = "UNIONBANK files should contain 'UB' reference and 14-digit numbers"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    logger.debug("Starting METROBANK validation")
                    # First, check for BANCNET or UNIONBANK markers and reject if found
                    for line in lines:
                        if '*' in line and len(line) >= 50:
                            is_valid = False
                            error_message = "This appears to be a BANCNET file. Please select BANCNET as the payment mode."
                            logger.debug("File identified as BANCNET")
                            break
                        # Block UNIONBANK if any line contains 'UB' and a 14-digit number
                        if 'UB' in line and re.search(r'\d{14}', line):
                            is_valid = False
                            error_message = "This appears to be a UNIONBANK file. Please select UNIONBANK as the payment mode."
                            logger.debug("File identified as UNIONBANK")
                            break
                    else:
                        # Only perform METROBANK-specific validation if not BANCNET or UNIONBANK
                        is_valid = False  # Reset is_valid for METROBANK validation
                        for line in lines:
                            if len(line.strip()) > 0:
                                logger.debug(f"Checking line for METROBANK pattern: {line}")
                                if re.search(r'\d{11,12}[A-Z]', line):
                                    logger.debug("Found METROBANK amount pattern")
                                    fields = [f.strip() for f in re.split(r'\s+', line.strip()) if f.strip()]
                                    logger.debug(f"METROBANK validation - Fields: {fields}")
                                    if len(fields) >= 2:
                                        logger.debug("Line has at least 2 fields")
                                        if (re.search(r'\d{11,12}[A-Z]', line) and re.search(r'\d{4,}', line)):
                                            is_valid = True
                                            logger.debug("METROBANK validation passed")
                                            break
                                        else:
                                            logger.debug("Line failed METROBANK pattern check")
                                    else:
                                        logger.debug("Line has fewer than 2 fields")
                                else:
                                    logger.debug("No METROBANK amount pattern found in line")
                        if not is_valid:
                            error_message = "METROBANK files should contain space-separated values with an amount in format 11-12 digits followed by a letter and an ATM reference"
                            logger.debug("METROBANK validation failed")

                elif payment_mode == 'PNB':
                    # PNB files should have caret-separated values with specific field patterns
                    for line in lines:
                        # First check if this is a ROB file by checking for both ^ and | separators
                        if '^' in line and '|' in line:
                            is_valid = False
                            error_message = "This appears to be a ROB file. Please select ROB as the payment mode."
                            logger.debug("File identified as ROB based on separators")
                            break
                            
                        if '^' in line:
                            fields = line.split('^')
                            # PNB files should have at least 7 fields and specific patterns
                            if len(fields) >= 7:
                                # Check for PNB-specific patterns:
                                # - Field 5 (index 4) should contain a reference number
                                # - Field 7 (index 6) should contain an amount
                                if (len(fields[4].strip()) >= 4 and  # Reference number field
                                        re.search(r'\d+', fields[4].strip()) and  # Contains digits
                                        re.search(r'\d+', fields[6].strip())):  # Amount field contains digits
                                    is_valid = True
                                    break
                    if not is_valid:
                        error_message = "PNB files should contain caret-separated values with specific field patterns (reference number in field 5 and amount in field 7)"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # CIS files should have caret-separated values with specific field patterns
                    for line in lines:
                        # First check if this is a ROB file by checking for both ^ and | separators
                        if '^' in line and '|' in line:
                            is_valid = False
                        ...
                        if '^' in line:
                            fields = line.split('^')
                            # CIS files should have at least 3 fields and specific patterns
                            if len(fields) >= 3:
                                # Check for CIS-specific patterns:
                                # - Field 1 (index 0) should contain a date
                                # - Field 2 (index 1) should contain a reference number
                                # - Field 3 (index 2) should contain an amount
                                if (re.search(r'\d{2}[/-]\d{2}[/-]\d{4}', fields[0].strip()) and  # Date field
                                        len(fields[1].strip()) >= 4 and  # Reference number field
                                        re.search(r'\d+', fields[1].strip()) and  # Contains digits
                                        re.search(r'\d+', fields[2].strip())):  # Amount field contains digits
                                    is_valid = True
                                    break
                    if not is_valid:
                        error_message = "CIS files should contain caret-separated values with specific field patterns (date in field 1, reference number in field 2, and amount in field 3)"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # CHINABANK files should have space-separated values with specific field patterns
                    for line in lines:
                        if len(line.strip()) > 0:
                            fields = [f.strip() for f in re.split(r'\s+', line.strip()) if f.strip()]
                            logger.debug(f"CHINABANK validation - Fields: {fields}")
                            # CHINABANK files should have at least 4 fields
                            if len(fields) >= 4:
                                # Check for CHINABANK-specific patterns:
                                # - Field 1 (index 0) should contain a date in MMDDYYYY format
                                # - Field 3 (index 2) should contain an amount
                                # - Field 4 (index 3) should contain an ATM reference
                                if (re.match(r'^\d{8}$', fields[0].strip()) and  # Date field in MMDDYYYY format
                                        re.search(r'\d+', fields[2].strip()) and  # Amount field should contain digits
                                        re.search(r'\d+',
                                                  fields[3].strip())):  # ATM reference field should contain digits
                                    is_valid = True
                                    logger.debug("CHINABANK validation passed")
                                    break
                    if not is_valid:
                        error_message = "CHINABANK files should contain space-separated values with specific field patterns (date in MMDDYYYY format in field 1, amount in field 3, and ATM reference in field 4)"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # CEBUANA files should have comma-separated values with specific field patterns
                    for line in lines:
                        if ',' in line:
                            fields = line.split(',')
                            logger.debug(f"CEBUANA validation - Fields: {fields}")
                            
                            # First check if this is an ECPAY file by checking field patterns
                            if len(fields) >= 7:
                                # ECPAY has ATM ref in index 5 and amount in index 6
                                if (re.search(r'\d+', fields[5].strip()) and  # ATM reference in index 5
                                    re.search(r'\d+', fields[6].strip())):    # Amount in index 6
                                    is_valid = False
                                    error_message = "This appears to be an ECPAY file. Please select ECPAY as the payment mode."
                                    logger.debug("File identified as ECPAY based on field patterns")
                                    break
                            
                            # CEBUANA files should have at least 7 fields
                            if len(fields) >= 7:
                                # Check for CEBUANA-specific patterns:
                                # - Field 1 (index 0) should contain a date or reference
                                # - Field 2 (index 1) should contain a reference or date
                                # - Field 5 (index 4) should contain an ATM reference
                                # - Field 7 (index 6) should contain an amount
                                if (len(fields[0].strip()) > 0 and  # First field should not be empty
                                        len(fields[1].strip()) > 0 and  # Second field should not be empty
                                        re.search(r'\d+',
                                                  fields[4].strip()) and  # ATM reference field should contain digits
                                        re.search(r'\d+', fields[6].strip())):  # Amount field should contain digits
                                    is_valid = True
                                    logger.debug("CEBUANA validation passed")
                                    break
                    if not is_valid:
                        error_message = "CEBUANA files should contain comma-separated values with at least 7 fields, including ATM reference in field 5 and amount in field 7"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # ECPAY files should have comma-separated values with specific field patterns
                    for line in lines:
                        if ',' in line:
                            fields = line.split(',')
                            logger.debug(f"ECPAY validation - Fields: {fields}")
                            # ECPAY files should have at least 7 fields
                            if len(fields) >= 7:
                                # Check for ECPAY-specific patterns:
                                # - Field 1 (index 0) should contain a date or reference
                                # - Field 2 (index 1) should contain a reference or date
                                # - Field 6 (index 5) should contain an ATM reference
                                # - Field 7 (index 6) should contain an amount
                                if (len(fields[0].strip()) > 0 and  # First field should not be empty
                                        len(fields[1].strip()) > 0 and  # Second field should not be empty
                                        re.search(r'\d+',
                                                  fields[5].strip()) and  # ATM reference field should contain digits
                                        re.search(r'\d+', fields[6].strip())):  # Amount field should contain digits
                                    is_valid = True
                                    logger.debug("ECPAY validation passed")
                                    break
                    if not is_valid:
                        error_message = "ECPAY files should contain comma-separated values with at least 7 fields, including ATM reference in field 6 and amount in field 7"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # SM files should have specific format with CS marker and fixed length
                    is_valid = False
                    for line in lines:
                        if len(line.strip()) > 0:
                            # First check if this is a METROBANK file by checking its structure
                            # METROBANK files have space-separated values with specific patterns
                            if re.search(r'\d{11,12}[A-Z]', line) and len(line.split()) >= 2:
                                is_valid = False
                                error_message = "This appears to be a METROBANK file. Please select METROBANK as the payment mode."
                                logger.debug("File identified as METROBANK based on structure")
                                break
                            
                            # Check for required SM file characteristics:
                            # 1. Line must be at least 45 characters long
                            # 2. Must contain 'CS' marker
                            # 3. Must have valid date format in positions 3-11 (MMDDYYYY)
                            # 4. Must have valid ATM reference in positions 18-31
                            if (len(line) >= 45 and
                                    'CS' in line and
                                    line[3:11].isdigit() and  # Check date format
                                    line[18:31].strip().isdigit()):  # Check ATM reference
                                ...
                                if amount_str and len(amount_str) > 0:
                                    is_valid = True
                                    break

                    if not is_valid:
                        error_message = "Invalid SM file format. File must contain lines with 'CS' marker, valid date format (MMDDYYYY), and valid ATM reference."
                        logger.error(f"SM validation failed: {error_message}")
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # BANCNET files should have asterisk markers
                    is_valid = False
                    for line in lines:
                        if '*' in line and len(line) >= 50:
                            is_valid = True
                            break
                    if not is_valid:
                        error_message = "BANCNET files should contain asterisk markers and be at least 50 characters long"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
                    ...
                    # ROB files should have both pipe and caret separators
                    is_valid = False
                    for line in lines:
                        if '|' in line and '^' in line:
                            is_valid = True
                            break
                    if not is_valid:
                        error_message = "ROB files should contain both pipe and caret separators"
                        try:
                            os.remove(file_path)
                        except Exception as e:
                            logger.error(f"Error deleting invalid file: {str(e)}")
            ...
            try:
                ...
                logger.error(f"Error cleaning up file after validation error: {str(cleanup_error)}")
            return jsonify({'error': 'Error validating file format. Please check if the file is readable and in the correct format.'}), 400

        # Store the payment mode in the processing status
        processing_status[processing_id] = {
            'status': 'processing',
        ...
        try:
            ...
            logger.error(f"Error cleaning up file after upload error: {str(cleanup_error)}")
    ...
    try:
        ...
        if payment_mode == 'CIS':
            ...
            for line in lines:
                ...
                if line.startswith('H UNIONBANK') or line.startswith('T'):
                    continue
            ...
            for line in lines:
                ...
                # Skip lines that don't have valid ATM reference or amount
                if len(line) < 45:  # Line too short to contain ATM ref
                    logger.debug(f"Skipping line too short for SM processing: {line}")
                    continue

                # Try to extract ATM reference
                atm_ref = line[18:31]  # Extract ATM reference
                if not atm_ref or not atm_ref.strip().isdigit():  # Check if ATM ref is valid
                    logger.debug(f"Skipping line with invalid ATM ref: {line}")
                    continue

                # Try to extract amount
                cs_pos = line.find('CS')
                if cs_pos <= 0:  # No CS marker found
                    logger.debug(f"Skipping line with no CS marker: {line}")
                    continue
                ...
                if not amount_str:  # No amount found
                    logger.debug(f"Skipping line with no valid amount: {line}")
                    continue

                # If we get here, the line has both valid ATM ref and amount
                raw_contents.append(line)
                ...
                # Process the amount we already found
