...
PAGE_SIZE = 1000  # Number of lines per page


@lru_cache(maxsize=128)
def detect_payment_mode(row):
    """Detect payment mode from a row of data with caching"""
...
def detect_amount(row):
    """Detect amount from a row of data with caching"""
...
def process_file_in_chunks(file_path, page=1, page_size=PAGE_SIZE):
    """Process file in chunks with pagination support"""
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        # Read the chunk
        chunk = list(itertools.islice(file, page_size))
        ...
        for line in chunk:
            # Process each line
            fields = line.strip().split()
            payment_mode = detect_payment_mode(tuple(fields))
            amount = detect_amount(tuple(fields))
            date = detect_date(fields)
...
@app.route('/api/process-file/<processing_id>', methods=['GET'])
def get_processed_chunk(processing_id):
    """Get a chunk of processed data with pagination"""
    ...
    file_path = processing_results[processing_id]['file_path']

    try:
        chunk = process_file_in_chunks(file_path, page, page_size)
        return jsonify({
            'data': chunk,
            'page': page,
            'page_size': page_size,
